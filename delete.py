...
def chessboard_homography():
	...
	if(len(list_of_pictures)!=0):
		...
		img2 = cv2.imread('/home/lorenzo/catkin_ws/src/chessboard_detection/src/camera_image3.jpeg',0)
		colour_img = cv2.imread('/home/lorenzo/catkin_ws/src/chessboard_detection/src/camera_image3'+str(counter)+'.jpeg')
	...
	# Else loop until a picture appears in that folder
		...
		while(len(list_of_pictures)==0):
			...
			time.sleep(5)
			list_of_pictures = sorted(glob.glob("/*.jpeg"),key=natural_keys)
			list_of_counters = []
			for picture in list_of_pictures:
				counter = picture.split('camera_image')[1]
				counter = counter.split('.')[0]
			...
			if(len(list_of_pictures)!=0):
				# Take the last picture taken and its number to process it
				counter = list_of_counters[-1]
				img2 = cv2.imread('/home/lorenzo/catkin_ws/src/chessboard_detection/src//camera_image'+str(counter)+'.jpeg',0)
	...
	if len(good)>MIN_MATCH_COUNT:
		...
		cv2.polylines(img2,[np.int32(dst)],True,0,20)
